...
const MOCK = {
  ...
  quotes: { close: 1212.1, pe_ttm: 18.4, pb: 5.59, total_mv: 148609700, dv_ttm: 2.5 },
  score: {
...
describe('StockDetailPanel', () => {
  ...
  it('formats market cap correctly (万元 → 万亿/亿)', async () => {
    (dbApi.getStockDetail as any).mockResolvedValue(MOCK);
    render(<StockDetailPanel ts_code="600519.SH" />);
    await waitFor(() => expect(screen.getByText('贵州茅台')).toBeTruthy());
    // total_mv=148609700 万元 = 14861 亿 = 1.49 万亿
    expect(screen.getByText(/1\.49 万亿/)).toBeTruthy();
  });
