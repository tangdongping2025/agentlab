    ...
    req.on('end', async () => {
      try {
        ...
        if (toolName === 'search' && !args.query) {
          ...
          res.end(JSON.stringify({ content: [{ type: 'text', text: '搜索必须提供 query 参数，请重新调用并传入搜索关键词。' }] }));
          return;
        }
        if (toolName === 'extract' && !args.url) {
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ content: [{ type: 'text', text: '网页提取必须提供 url 参数，请重新调用并传入目标URL。' }] }));
          return;
        }
